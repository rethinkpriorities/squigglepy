...
def check_correlation_from_parameter(a, b, corr):
    samples = np.column_stack([a @ 1000, b @ 1000])
    estimated_corr = stats.spearmanr(samples).statistic
...
def random_distributions(draw):
    ...
    if i == 0:
        # Uniform
        a = draw(st.floats(allow_infinity=False, allow_nan=False))
        b = draw(st.floats(min_value=a, allow_infinity=False, allow_nan=False))
