...
def test_simple_bayes():
    out = simple_bayes(prior=0.01, likelihood_h=0.8, likelihood_not_h=0.096)
...
def test_bayesnet_conditional():
    ...
    set_seed(42)
    out = bayesnet(define_event, find=lambda e: e["a"] == 1, n=100)
...
def test_bayesnet_raw():
    out = bayesnet(lambda: {"a": 1, "b": 2}, find=lambda e: e["a"], raw=True, n=100)
    assert out == [1] * 100
...
def test_bayesnet_cache_n_error():
    ...
    with pytest.raises(ValueError) as excinfo:
        bayesnet(define_event, find=lambda e: e["a"], n=1000)
...
def test_bayesnet_cachefile(cachefile):
    ...
    bayesnet(define_event, find=lambda e: e["a"], dump_cache_file=cachefile, n=100)

    out = bayesnet(define_event, find=lambda e: e["a"], raw=True, n=100)
    ...
    out = bayesnet(define_event, find=lambda e: e["a"], memcache=False, raw=True, n=100)
...
def test_bayesnet_cachefile_insufficent_samples_error(cachefile):
    ...
    with pytest.raises(ValueError) as excinfo:
        bayesnet(define_event, load_cache_file=cachefile, find=lambda e: e["a"], n=1000)
    assert "insufficient samples" in str(excinfo.value)
...
def test_bayesnet_multicore():
    ...
    out = bayesnet(define_event, find=lambda e: e["a"], cores=2, n=100)
    assert out == 1
