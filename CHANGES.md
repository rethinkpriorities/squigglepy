...
- **[Breaking change]** The signature of `dist_fn` has changed to make it simpler:
  - `dist_fn` no longer takes `None` positional arguments. (i.e. `dist_fn(dist1, None, np.ceil)` is now `dist_fn(dist1, np.ceil)`).
  - `dist_fn` no longer supports passing arbitrary arguments as if they were distributions. Use partials instead (i.e. `dist_fn(d1, 2, round)` is now `dist_fn(d1, partial(round, digits=2))`).
